...
// =====================
// 字节序转换
// Polyglot book 文件是大端序，
// 而大多数 PC 是小端序，所以读出来后要翻转
// =====================
...
// Polyglot 随机数表
// 这一大坨常量建议放 cpp，别放头文件
// 你把原来的 kPolyglotRandom 整段搬到这里即可
// =====================
namespace {

constexpr std::array<uint64_t, 781> kPolyglotRandom = {{
...
// =====================
// 计算当前局面的 Polyglot hash
// 这个 hash 必须严格按 Polyglot 规范来，
// 否则就无法和 .bin 开局库匹配
// =====================
uint64_t polyglot_hash(const Position& pos) {
    ...
    for (int sq = 0; sq < 64; ++sq) {
        ...
        int pi = poly_piece_index(p);
        if (pi >= 0)
            h ^= kPolyglotRandom[64 * pi + sq];
    }

    // 易位权
    if (pos.castlingRights & CR_WK) h ^= kPolyglotRandom[768 + 0];
    if (pos.castlingRights & CR_WQ) h ^= kPolyglotRandom[768 + 1];
    if (pos.castlingRights & CR_BK) h ^= kPolyglotRandom[768 + 2];
    if (pos.castlingRights & CR_BQ) h ^= kPolyglotRandom[768 + 3];

    // 吃过路兵列
    // 注意 Polyglot 不是“只要 epSquare 存在就异或”，
    // 而是必须真的存在可执行的 EP 吃子，才把该 file 混进去
    if (pos.epSquare != -1) {
        const int ep = pos.epSquare;
        ...
        if (pos.side == WHITE) {
            if (f > 0) {
                int s = ep - 9;
                if ((unsigned)s < 64u && pos.board[s] == W_PAWN)
                    canCapture = true;
            }
            if (f < 7) {
                int s = ep - 7;
            ...
            if (f > 0) {
                int s = ep + 7;
                if ((unsigned)s < 64u && pos.board[s] == B_PAWN)
                    canCapture = true;
            }
            if (f < 7) {
                int s = ep + 9;
        ...
        if (canCapture)
            h ^= kPolyglotRandom[772 + f];
    }

    // 轮到白方走时异或 side key
    if (pos.side == WHITE)
        h ^= kPolyglotRandom[780];

    return h;
}

// =====================
// Polyglot 原始 16-bit 走法 -> 引擎内部合法走法
// 这里不能只靠 from/to 直接构造，
// 因为你的引擎内部走法编码、易位表示、升变表示可能不同。
// 最稳的办法：
// 1. 解码出 from/to/promo
// 2. 生成当前所有合法步
// 3. 在合法步里找匹配项
...
Move decode_raw_to_legal(const Position& pos, uint16_t raw) {
    ...
    int from = make_sq(fromFile, fromRank);
    int to   = make_sq(toFile, toRank);

    // Polyglot 对易位的编码是“王吃车格”
    // 你的引擎内部更可能是 E1->G1 / E1->C1 这种
    ...
    moves.reserve(256);
    movegen::generate_legal(pcopy, moves);
...
bool Book::load(const std::string& path) {
    ...
    entries_.resize((size_t)(sz / 16));

    for (size_t i = 0; i < entries_.size(); ++i) {
        Entry e{};

        in.read(reinterpret_cast<char*>(&e.key), sizeof(e.key));
        in.read(reinterpret_cast<char*>(&e.rawMove), sizeof(e.rawMove));
        in.read(reinterpret_cast<char*>(&e.weight), sizeof(e.weight));
        in.read(reinterpret_cast<char*>(&e.learn), sizeof(e.learn));
        ...
        // Polyglot 文件是大端序，读完要翻
        e.key     = bswap64(e.key);
        e.rawMove = bswap16(e.rawMove);
        e.weight  = bswap16(e.weight);
        e.learn   = bswap32(e.learn);
...
bool Book::loaded() const {
    return loaded_ && !entries_.empty();
}

// =====================
// 在当前局面下，从开局库中选权重最高的合法走法
// =====================
Move Book::best_move(const Position& pos, uint16_t* outWeight) const {
    if (!loaded())
        return 0;

    const uint64_t key = polyglot_hash(pos);
    auto [l, r] = equal_range(key);

    Move best = 0;
    uint16_t bestW = 0;

    for (size_t i = l; i < r; ++i) {
        Move m = decode_raw_to_legal(pos, entries_[i].rawMove);
        if (!m)
            continue;

        if (!best || entries_[i].weight > bestW) {
            best = m;
            bestW = entries_[i].weight;
        }
    }
...
// =====================
// 从 first 开始，一步步沿着开局库往后拼 PV
// 注意这里用的是 Position 的值传递，
// 所以不会改到外部真实局面
// =====================
std::vector<Move> Book::build_pv(Position pos, Move first, int maxPly) const {
    std::vector<Move> pv;
    if (!first)
        return pv;

    pv.push_back(first);
    Undo u = pos.do_move(first);
    (void)u;

    for (int ply = 1; ply < maxPly; ++ply) {
        Move m = best_move(pos, nullptr);
        if (!m)
            break;

        pv.push_back(m);
        Undo uu = pos.do_move(m);
        (void)uu;
    }
...
// =====================
// 在已排序 entries_ 中查找 key 对应的区间
// Polyglot 文件标准上通常按 key 排序
// 所以这里能直接二分
// =====================
std::pair<size_t, size_t> Book::equal_range(uint64_t key) const {
    auto itL = std::lower_bound(
    ...
    return {
        size_t(itL - entries_.begin()),
        size_t(itR - entries_.begin())
    };
}

// 只是做一层本地转发
std::string move_to_uci_local(Move m) {
    return search::move_to_uci(m);
}
